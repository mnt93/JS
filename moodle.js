...
function plot_distribution_plotly(div_id, law, params, x_obs, alpha, side, title, options) {
    ...
    if (!isNaN(x_obs)) {
        annotations.push(_make_ann(x_obs, '$' + x_title + '_{\\text{obs}} = ' + (Math.round(x_obs * 1000) / 1000) + '$', C_OBS, 0, -55));
    }
    if (!isNaN(x_crit_lo)) {
        annotations.push(_make_ann(x_crit_lo, '$' + x_title + '_{\\text{crit}} = ' + (Math.round(x_crit_lo * 1000) / 1000) + '$', C_CRIT, -45, -45));
    }
    if (!isNaN(x_crit_hi)) {
        annotations.push(_make_ann(x_crit_hi, '$' + x_title + '_{\\text{crit}} = ' + (Math.round(x_crit_hi * 1000) / 1000) + '$', C_CRIT, 45, -45));
    }
