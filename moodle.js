...
function plot_distribution_plotly(div_id, law, params, x_obs, alpha, side, title, options) {
    ...
    var _make_ann = function(xv, label, color, ax, ay) {
        return {
            x: xv, y: 0, xref: 'x', yref: 'y',
            text: label,
    ...
    if (!isNaN(x_obs)) {
        annotations.push(_make_ann(x_obs, x_title + '_obs = ' + (Math.round(x_obs * 1000) / 1000), C_OBS, 0, -55));
    }
    if (!isNaN(x_crit_lo)) {
        annotations.push(_make_ann(x_crit_lo, x_title + '_crit = ' + (Math.round(x_crit_lo * 1000) / 1000), C_CRIT, -45, -45));
    }
    if (!isNaN(x_crit_hi)) {
        annotations.push(_make_ann(x_crit_hi, x_title + '_crit = ' + (Math.round(x_crit_hi * 1000) / 1000), C_CRIT, 45, -45));
    }
    ...
    var plotly_opts = { responsive: true, displayModeBar: false };
