...
function plot_distribution(div_id, law, params, x_obs, alpha, side, title, options) {
    ...
    google.charts.setOnLoadCallback(function() {
        ...
        var ann_fn = function(x) {
            ...
            if (!isNaN(x_sym)     && xr === Math.round(x_sym))     return vname + '_sym=' + (is_discrete ? x_sym : Math.round(x_sym * 1000) / 1000);
            if (!isNaN(x_crit_lo) && xr === Math.round(x_crit_lo)) return vname + '_crit=' + (is_discrete ? x_crit_lo : Math.round(x_crit_lo * 1000) / 1000);
            if (!isNaN(x_crit_hi) && xr === Math.round(x_crit_hi)) return vname + '_crit=' + (is_discrete ? x_crit_hi : Math.round(x_crit_hi * 1000) / 1000);
            return '';
        };
