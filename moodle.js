...
function plot_distribution_plotly(div_id, law, params, x_obs, alpha, side, title, options) {
    ...
    /* ------------------------------------------------------------------ */
        ...
        /* ---- Zones hachurées continues : lignes diagonales manuelles ----
           fillpattern n'est pas supporté sur scatter+fill dans Plotly.
           Approche : fond semi-transparent + lignes diagonales à pas fixe. */

        /* Génère les traces de hachures pour un intervalle [xa, xb] de la courbe */
        var _add_hatch_zone = function(xa, xb) {
            /* 1. Fond ambré semi-transparent */
            var xZ = [], yZ = [];
            var hz = 0;
            ...
            if (!xZ.length) return;
            traces.push({
                x: [xZ[0]].concat(xZ).concat([xZ[xZ.length - 1]]),
                y: [0].concat(yZ).concat([0]),
                type: 'scatter', mode: 'lines', fill: 'tozeroy',
                fillcolor: 'rgba(239,159,39,0.22)',
                line: { color: 'rgba(0,0,0,0)', width: 0 },
                name: '', hoverinfo: 'skip', showlegend: false
            });
            /* 2. Lignes diagonales à pas fixe dans l'intervalle */
            var n_lines = 18;
            var x_span  = xb - xa;
            var hatch_x = [], hatch_y = [];
            var li = 0;
            while (_lte(li, n_lines)) {
                var x_start = xa + (x_span / n_lines) * li;
                /* Chercher y_max de la courbe à x_start (interpolation linéaire) */
                var y_top = 0;
                var fi = 0;
                while (_lt(fi, xs.length - 1)) {
                    if (_lte(xs[fi], x_start) && _gte(xs[fi + 1], x_start)) {
                        var t = (_gt(xs[fi + 1] - xs[fi], 0))
                                ? (x_start - xs[fi]) / (xs[fi + 1] - xs[fi]) : 0;
                        y_top = ys[fi] + t * (ys[fi + 1] - ys[fi]);
                        break;
                    }
                    fi++;
                }
                /* Ligne de (x_start, 0) à (x_start, y_top) */
                hatch_x.push(x_start); hatch_x.push(x_start); hatch_x.push(null);
                hatch_y.push(0);       hatch_y.push(y_top);    hatch_y.push(null);
                li++;
            }
            traces.push({
                x: hatch_x, y: hatch_y,
                type: 'scatter', mode: 'lines',
                line: { color: 'rgba(226,75,74,0.55)', width: 1 },
                name: '', hoverinfo: 'skip', showlegend: false
            });
        };
        ...
        if (!isNaN(x_obs)) {
            ...
            if (side === 'right' || side === 'bilateral') {
                _add_hatch_zone(x_obs, hatch_hi_end);
            }
            if (side === 'bilateral' && !isNaN(x_sym)) {
                _add_hatch_zone(hatch_lo_end, x_sym);
            }
            if (side === 'left') {
                _add_hatch_zone(hatch_lo_end, x_obs);
            }
        }
