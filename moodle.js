...
function plot_distribution(div_id, law, params, x_obs, alpha, side, title, options) {
    ...
    var x_title  = opts.x_title  || '';
    var y_title  = opts.y_title  || '';
    var n_points = opts.n_points || 200;
    ...
    /* Valeur symetrique : plus grand x_sym tel que P(X=x_sym) <= P(X=x_obs)
       (cas bilateral uniquement, loi discrete) ou
       x tel que F(x) = 1 - F(x_obs) (loi continue) */
    var x_sym = NaN;
    if (side === 'bilateral') {
        if (is_discrete) {
            var p_obs = pdf_fn(Math.round(x_obs));
            var mu_d  = (law === 'binomial') ? params[0] * params[1] : params[0];
            /* Chercher du cote oppose a x_obs par rapport a la moyenne */
            if (Math.sign(x_obs - mu_d) === 1) {
                /* x_obs est a droite -> chercher a gauche */
                var j = Math.floor(mu_d);
                while (j > 0 && pdf_fn(j) > p_obs) j--;
                x_sym = j;
            } else {
                /* x_obs est a gauche -> chercher a droite */
            ...
            /* Loi continue : symetrique par rapport a la mediane */
            var med = inv_fn(0.5);
            x_sym = 2 * med - x_obs;
        }
    }

    /* Coloration : pas de couleur speciale pour x_obs */
    ...
    /* Etendre si x_obs ou x_sym sont hors bornes */
    ...
    google.charts.setOnLoadCallback(function() {
        ...
        /* Annotation au-dessus d'une barre ou d'un point */
        var ann_fn = function(x) {
            var xr = is_discrete ? Math.round(x) : x;
            if (!isNaN(x_obs) && xr === Math.round(x_obs))               return 'x=' + x_obs;
            if (!isNaN(x_sym) && xr === Math.round(x_sym))               return 'x=' + (is_discrete ? x_sym : Math.round(x_sym * 1000) / 1000);
            if (!isNaN(x_crit_lo) && xr === Math.round(x_crit_lo))       return 'c=' + (is_discrete ? x_crit_lo : Math.round(x_crit_lo * 1000) / 1000);
            if (!isNaN(x_crit_hi) && xr === Math.round(x_crit_hi))       return 'c=' + (is_discrete ? x_crit_hi : Math.round(x_crit_hi * 1000) / 1000);
            return '';
        };

        if (is_discrete) {

            /* Loi discrete : ColumnChart avec annotations */
            var data = [['k', y_title || 'P(X=k)', {role: 'style'}, {role: 'annotation'}]];
            var k = Math.round(x_min);
            while (k !== Math.round(x_max) + 1) {
                data.push([k, pdf_fn(k), color_fn(k), ann_fn(k)]);
                k = k + 1;
            }
            var table = google.visualization.arrayToDataTable(data);
            new google.visualization.ColumnChart(el).draw(table, {
                ...
                annotations: {alwaysOutside: true, textStyle: {fontSize: 9, bold: true, color: '#2c3e50'}},
                width: w, height: h, chartArea: {left: 60, top: 40, width: '80%', height: '65%'}
            });

        } else {

            /* Loi continue : AreaChart (courbe continue remplie) */
            /* Deux series : zone de non-rejet (bleu) et zone de rejet (rouge) */
            var data_cont = [['x', 'Non-rejet', 'Rejet',
                              {role: 'annotation'}, {role: 'annotationText'}]];

            var step  = (x_max - x_min) / n_points;
            var idx_i = 0;
            /* Positions des annotations (x_obs, x_sym, x_crit_lo, x_crit_hi) */
            var ann_targets = [];
            if (!isNaN(x_obs))     ann_targets.push({x: x_obs,     lbl: 'obs=' + (Math.round(x_obs * 100) / 100)});
            if (!isNaN(x_sym))     ann_targets.push({x: x_sym,     lbl: 'sym=' + (Math.round(x_sym * 100) / 100)});
            if (!isNaN(x_crit_lo)) ann_targets.push({x: x_crit_lo, lbl: 'c='  + (Math.round(x_crit_lo * 100) / 100)});
            if (!isNaN(x_crit_hi)) ann_targets.push({x: x_crit_hi, lbl: 'c='  + (Math.round(x_crit_hi * 100) / 100)});

            /* Trouver le point le plus proche de chaque cible */
            var ann_map = {};
            ann_targets.forEach(function(t) {
                var best_idx = Math.round((t.x - x_min) / step);
                if (Math.sign(best_idx) === -1) best_idx = 0;
                if (Math.sign(best_idx - n_points) === 1) best_idx = n_points;
                ann_map[best_idx] = t.lbl;
            });

            while (idx_i !== n_points + 1) {
                var xi = x_min + idx_i * step;
                var yi = pdf_fn(xi);
                if (isFinite(yi) && Math.sign(yi) !== -1) {
                    var is_rej = (!isNaN(x_crit_lo) && Math.sign(xi - x_crit_lo) !== 1) ||
                                 (!isNaN(x_crit_hi) && Math.sign(xi - x_crit_hi) !== -1);
                    var ann_lbl  = ann_map[idx_i] ? ann_map[idx_i] : null;
                    var ann_text = ann_lbl ? ann_lbl : null;
                    if (is_rej) {
                        data_cont.push([xi, 0, yi, ann_lbl, ann_text]);
                    } else {
                        data_cont.push([xi, yi, 0, ann_lbl, ann_text]);
                    }
                }
                idx_i = idx_i + 1;
            }
            ...
            new google.visualization.AreaChart(el).draw(table_cont, {
                title: title, titleTextStyle: {fontSize: 13, color: '#2c3e50'},
                hAxis: {title: x_title, titleTextStyle: {color: '#555'}, textStyle: {fontSize: 10}},
                vAxis: {title: y_title || 'f(x)', titleTextStyle: {color: '#555'}, minValue: 0},
                series: {
                    0: {color: '#2980b9', areaOpacity: 0.35, lineWidth: 2},
                    1: {color: '#e74c3c', areaOpacity: 0.35, lineWidth: 2}
                },
                legend: {position: 'none'},
                annotations: {
                    textStyle: {fontSize: 9, bold: true, color: '#2c3e50'},
                    stemColor: '#888', stemLength: 8
                },
                width: w, height: h, chartArea: {left: 60, top: 40, width: '80%', height: '65%'}
            });
        }
        ...
        /* Legende */
        var legend_items = [
            {color: C_REJECT,   label: 'Zone de rejet'},
            {color: C_NOREJECT, label: 'Zone de non-rejet'}
        ];
        var legendDiv = document.createElement('div');
        legendDiv.style.cssText = 'display:flex; flex-wrap:wrap; gap:6px 14px;'
            + ' padding:6px 8px; font-size:12px; font-family:Arial,sans-serif;'
