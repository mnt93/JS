...
function plot_distribution_plotly(div_id, law, params, x_obs, alpha, side, title, options) {
    ...
    /* ------------------------------------------------------------------ */
        ...
        /* Espacement fixe = 1/20 de la largeur totale du graphique.        */
        /* Les traits sont placees aux positions globales qui tombent dans   */
        /* [xa, xb], sans couleur de fond supplementaire.                   */
        var _add_hatch_zone = function(xa, xb) {
            /* Espacement global : 1/20 de (x_max - x_min) */
            var step_h  = (x_max - x_min) / 20;
            /* Premier trait : multiple de step_h >= xa */
            var x_first = Math.ceil(xa / step_h) * step_h;
            var hatch_x = [], hatch_y = [];
            var xv = x_first;
            while (_lte(xv, xb)) {
        ...
        /* Hachures : de x_obs jusqu'au bord (x_obs peut depasser x_crit) */
        if (!isNaN(x_obs)) {
            if (side === 'right' || side === 'bilateral') {
                _add_hatch_zone(x_obs, x_max);
            }
            if (side === 'bilateral' && !isNaN(x_sym)) {
                _add_hatch_zone(x_min, x_sym);
            }
            if (side === 'left') {
                _add_hatch_zone(x_min, x_obs);
            }
        }
