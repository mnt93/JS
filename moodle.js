...
/*
/*  Ces fonctions encapsulent le tracé Google Charts ColumnChart pour les exercices
/*  de test binomial (BINOM_MENDEL_CLIN). Elles sont appelées depuis les corrections
/*  des exercices XML. Google Charts (loader.js) doit être chargé dans le CDN.
/*
/*  Signatures :
/*
...
/*    @param p0        {number}   Probabilité de succès sous H0.
/*    @param mu_H0     {number}   Espérance n*p0 (calcul via entiers, sans artefact flottant).
/*    @param sigma_H0  {number}   Écart-type sqrt(n*p0*(1-p0)).
/*    @param k_obs     {number}   Valeur observée (annotée k_obs sur la figure).
/*    @param k_sym     {number}   Valeur symétrique (annotée k_sym sur la figure).
...
/*    @param options   {object}   Options facultatives : width, height (défauts 560×270).
/*
/*  plot_normal_01(div_id, z_obs, color_fn, title, options)
/*    Trace la densité de N(0,1) sur [-4, 4] avec des barres colorées.
/*    @param div_id    {string}   ID du <div> cible dans le DOM.
/*    @param z_obs     {number}   Statistique de test z_obs (valeur numérique).
