...
function plot_distribution(div_id, law, params, x_obs, alpha, side, title, options) {
    ...
    if (is_discrete) {
        /* Recherche entiere des valeurs critiques */
        var n_max = (law === 'binomial') ? params[0] : Math.round(params[0] * 10);
        if (side === 'bilateral' || side === 'left') {
            var ki = 0;
            while (ki < n_max && cdf_fn(ki) <= alpha / (side === 'bilateral' ? 2 : 1)) ki++;
            x_crit_lo = ki - 1;
        }
        if (side === 'bilateral' || side === 'right') {
            var ks = n_max;
            while (ks > 0 && (1 - cdf_fn(ks - 1)) <= alpha / (side === 'bilateral' ? 2 : 1)) ks--;
            x_crit_hi = ks + 1;
        }
    } else {
        if (side === 'bilateral') {
    ...
    if (opts.x_min !== undefined && opts.x_max !== undefined) {
        ...
        var sd_d  = (law === 'binomial') ? Math.sqrt(params[0] * params[1] * (1 - params[1]))
                                         : Math.sqrt(params[0]);
        x_min = Math.max(0, Math.floor(mu_d - 4 * sd_d));
        x_max = Math.min((law === 'binomial' ? params[0] : mu_d + 6 * sd_d),
    ...
    /* ── Libelles de legende selon le test ── */
    var lbl_reject = side === 'bilateral'
        ? 'Zone de rejet (|x| >= valeur critique, alpha = ' + alpha + ')'
        : side === 'left'
        ? 'Zone de rejet (x <= valeur critique, alpha = ' + alpha + ')'
        : 'Zone de rejet (x >= valeur critique, alpha = ' + alpha + ')';
    ...
    google.charts.setOnLoadCallback(function() {
        ...
        if (is_discrete) {
            data = [['k', y_title || 'P(X=k)', {role: 'style'}, {role: 'annotation'}]];
            var k = Math.round(x_min);
            while (k !== Math.round(x_max) + 1) {
                data.push([k, pdf_fn(k), color_fn(k), k === Math.round(x_obs) ? String(k) : '']);
                k = k + 1;
            }
        } else {
            data = [['x', y_title || 'f(x)', {role: 'style'}]];
            var step = (x_max - x_min) / n_points;
            var idx  = 0;
            while (idx !== n_points + 1) {
                var xi = x_min + idx * step;
                var yi = pdf_fn(xi);
                if (isFinite(yi) && Math.sign(yi) !== -1) data.push([xi, yi, color_fn(xi)]);
                idx = idx + 1;
            }
        }
        ...
        var chart_opts = {
            ...
            legend:      {position: 'none'},
            bar:         {groupWidth: is_discrete ? '80%' : '100%'},
            annotations: {alwaysOutside: false, textStyle: {fontSize: 9, bold: true}},
            width:       w,
            height:      h,
            chartArea:   {left: 60, top: 40, width: '80%', height: '65%'}
        ...
        new google.visualization.ColumnChart(el).draw(table, chart_opts);

        /* ── Legende ── */
        var legend_items = [
            {color: C_REJECT,   label: lbl_reject},
            {color: C_NOREJECT, label: 'Zone de non-rejet'},
            {color: C_OBS,      label: 'Valeur observee : ' + x_obs}
        ];
        var legendDiv = document.createElement('div');
        legendDiv.style.cssText = 'display:flex; flex-wrap:wrap; gap:6px 14px;'
            + ' padding:6px 8px; font-size:12px; font-family:Arial,sans-serif;'
