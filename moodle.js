...
function plot_distribution_plotly(div_id, law, params, x_obs, alpha, side, title, options) {
    ...
    var legend_traces = [
        ...
        },
        {
            x: [null], y: [null], type: 'scatter', mode: 'markers',
            marker: {
                color: 'rgba(239,159,39,0.18)', size: 14, symbol: 'square',
                line: { color: 'rgba(226,75,74,0.8)', width: 1 }
            },
            name: 'p-valeur (zones au moins aussi extrêmes)',
            showlegend: true, hoverinfo: 'skip'
        }
    ];
    ...
    if (is_discrete) {
        ...
        var pmf_arr = [];
        var col_arr = [];
        var pat_arr = [];   /* fillpattern par barre */
        ...
        while (_lte(k_iter, Math.round(x_max))) {
            ...
            /* "au moins aussi extreme" = pmf(k) <= pmf(x_obs), k != x_obs   */
            var is_extreme = (!isNaN(x_obs) && _lte(pmf_k, p_obs_d)
                              && k_iter !== Math.round(x_obs));

            if (is_rej && is_extreme) {
                col_arr.push(C_REJECT_BAR);
                pat_arr.push({ shape: '/', size: 6,
                               fgcolor: 'rgba(239,159,39,0.95)',
                               bgcolor: 'rgba(226,75,74,0.70)', fgopacity: 1 });
            } else if (is_rej) {
                col_arr.push(C_REJECT_BAR);
                pat_arr.push({});
            } else if (is_extreme) {
                ...
                pat_arr.push({ shape: '/', size: 6,
                               fgcolor: 'rgba(226,75,74,0.9)',
                               bgcolor: 'rgba(55,138,221,0.55)', fgopacity: 1 });
            } else {
                col_arr.push(C_NOREJECT_BAR);
                pat_arr.push({});
            }
    ...
    /* ------------------------------------------------------------------ */
        ...
        /* Les hachures sont construites en premier et placées SOUS la courbe */
        /* La courbe principale est ajoutée après avec un fill semi-transparent */
        var hatch_traces = [];

        /* Génère les traces de hachures pour un intervalle [xa, xb] */
        /* Hachures partant de xa vers xb (ou de xb vers x_min selon le sens) */
        /* Espacement = 1/20 de la largeur totale, premier trait en xa       */
        var _add_hatch_zone = function(xa, xb, from_right) {
            var step_h = (x_max - x_min) / 20;
            var hatch_x = [], hatch_y = [];
            /* from_right=true : partir de xb vers xa (valeur symetrique)   */
            /* from_right=false: partir de xa vers xb (valeur observee)      */
            var origin = from_right ? xb : xa;
            var xv     = origin;
            var going_left = from_right ? true : false;
            while (going_left ? _gte(xv, xa) : _lte(xv, xb)) {
                /* Interpolation de y sur la courbe */
                var ytop = 0, fi = 0;
                while (_lt(fi, xs.length - 1)) {
                    if (_lte(xs[fi], xv) && _gte(xs[fi + 1], xv)) {
                        var dx   = xs[fi + 1] - xs[fi];
                        var frac = _gt(dx, 0) ? (xv - xs[fi]) / dx : 0;
                        ytop = ys[fi] + frac * (ys[fi + 1] - ys[fi]);
                        break;
                    }
                    fi++;
                }
                hatch_x.push(xv); hatch_x.push(xv); hatch_x.push(null);
                hatch_y.push(0);   hatch_y.push(ytop); hatch_y.push(null);
                xv = going_left ? xv - step_h : xv + step_h;
            }
            if (!hatch_x.length) return;
            hatch_traces.push({
                x: hatch_x, y: hatch_y,
                type: 'scatter', mode: 'lines',
                line: { color: 'rgba(200,50,50,0.90)', width: 3 },
                name: '', hoverinfo: 'skip', showlegend: false
            });
        };

        /* Hachures partant de x_obs vers l'extreme droit                    */
        /* et (bilateral) de x_sym vers l'extreme gauche                    */
        if (!isNaN(x_obs)) {
            if (side === 'right' || side === 'bilateral') {
                /* Partir de x_obs, aller vers x_max */
                _add_hatch_zone(x_obs, x_max, false);
            }
            if (side === 'bilateral' && !isNaN(x_sym)) {
                /* Partir de x_sym, aller vers x_min */
                _add_hatch_zone(x_min, x_sym, true);
            }
            if (side === 'left') {
                /* Partir de x_obs, aller vers x_min */
                _add_hatch_zone(x_min, x_obs, true);
            }
        }

        /* Courbe principale APRES les hachures -> dessinée par-dessus */
        /* fillcolor transparent pour laisser voir les hachures en dessous */
        var traces = [{
            x: xs, y: ys,
        ...
        /* Fusionner : hachures d'abord, puis courbe par-dessus */
        traces = hatch_traces.concat(traces);
        ...
        /* Ligne verticale pointillee : valeur observee */
        if (!isNaN(x_obs)) {
