...
function plot_distribution_plotly(div_id, law, params, x_obs, alpha, side, title, options) {
    ...
    if (!isNaN(x_obs)) {
        legend_traces.push({
            x: [null], y: [null], type: 'scatter', mode: 'markers',
            marker: { color: C_OBS, size: 10, symbol: 'line-ns',
                      line: { color: C_OBS, width: 2 } },
            name: _plain_ann('obs', x_obs),
            showlegend: true, hoverinfo: 'skip'
        });
    }
    if (!isNaN(x_crit_lo) || !isNaN(x_crit_hi)) {
        var crit_val = !isNaN(x_crit_lo) ? x_crit_lo : x_crit_hi;
        legend_traces.push({
            x: [null], y: [null], type: 'scatter', mode: 'markers',
            marker: { color: C_CRIT, size: 10, symbol: 'line-ns',
                      line: { color: C_CRIT, width: 2 } },
            name: _plain_ann('crit', crit_val),
            showlegend: true, hoverinfo: 'skip'
        });
    }
    ...
    if (is_discrete) {
        ...
        while (_lte(k_iter, Math.round(x_max))) {
            ...
            /* "au moins aussi extreme" = pmf <= pmf_obs, hors zone de rejet */
            var is_extreme = (!isNaN(x_obs) && !is_rej && _lte(pmf_k, p_obs_d)
            ...
            if (is_rej) {
                ...
                pat_arr.push(_hatch('rgba(226,75,74,0.8)', 'rgba(55,138,221,0.3)'));
            } else {
                col_arr.push(C_NOREJECT_BAR);
                pat_arr.push({});
            }
    ...
    /* ------------------------------------------------------------------ */
        ...
        /* Trace principal : courbe + aire non-rejet */
        var traces = [{
            x: xs, y: ys,
        ...
        /* ---- Zones hachurées : au moins aussi extrêmes que x_obs ----   */
        /* Pour test unilatéral droit  : x >= x_obs (hors zone rejet)      */
        /* Pour test unilatéral gauche : x <= x_obs (hors zone rejet)      */
        /* Pour test bilatéral         : x >= x_obs OU x <= x_sym          */
        if (!isNaN(x_obs)) {

            /* Segment haché droit : entre x_obs et x_crit_hi (ou x_obs et x_max) */
            var hatch_hi_end = !isNaN(x_crit_hi) ? x_crit_hi : x_max;
            var hatch_lo_end = !isNaN(x_crit_lo) ? x_crit_lo : x_min;

            /* Zone haché côté x_obs */
            if (side === 'right' || side === 'bilateral') {
                var xH1 = [], yH1 = [];
                var hh = 0;
                while (_lt(hh, xs.length)) {
                    if (_gte(xs[hh], x_obs) && _lte(xs[hh], hatch_hi_end)) {
                        xH1.push(xs[hh]); yH1.push(ys[hh]);
                    }
                    hh++;
                }
                if (xH1.length) {
                    traces.push({
                        x: [xH1[0]].concat(xH1).concat([xH1[xH1.length - 1]]),
                        y: [0].concat(yH1).concat([0]),
                        type: 'scatter', mode: 'lines', fill: 'tozeroy',
                        fillcolor: 'rgba(239,159,39,0.18)',
                        fillpattern: _hatch('rgba(226,75,74,0.7)', 'rgba(239,159,39,0.0)'),
                        line: { color: 'rgba(0,0,0,0)', width: 0 },
                        hoverinfo: 'skip', showlegend: false
                    });
                }
            ...
            /* Zone hachée côté x_sym (test bilatéral) */
            if (side === 'bilateral' && !isNaN(x_sym)) {
                var xH2 = [], yH2 = [];
                var hh2 = 0;
                while (_lt(hh2, xs.length)) {
                    if (_lte(xs[hh2], x_sym) && _gte(xs[hh2], hatch_lo_end)) {
                        xH2.push(xs[hh2]); yH2.push(ys[hh2]);
                    }
                    hh2++;
                }
                if (xH2.length) {
                    traces.push({
                        x: [xH2[0]].concat(xH2).concat([xH2[xH2.length - 1]]),
                        y: [0].concat(yH2).concat([0]),
                        type: 'scatter', mode: 'lines', fill: 'tozeroy',
                        fillcolor: 'rgba(239,159,39,0.18)',
                        fillpattern: _hatch('rgba(226,75,74,0.7)', 'rgba(239,159,39,0.0)'),
                        line: { color: 'rgba(0,0,0,0)', width: 0 },
            ...
            /* Zone hachée test unilatéral gauche */
            if (side === 'left') {
                var xH3 = [], yH3 = [];
                var hh3 = 0;
                while (_lt(hh3, xs.length)) {
                    if (_lte(xs[hh3], x_obs) && _gte(xs[hh3], hatch_lo_end)) {
                        xH3.push(xs[hh3]); yH3.push(ys[hh3]);
                    }
                    hh3++;
                }
                if (xH3.length) {
                    traces.push({
                        x: [xH3[0]].concat(xH3).concat([xH3[xH3.length - 1]]),
                        y: [0].concat(yH3).concat([0]),
                        type: 'scatter', mode: 'lines', fill: 'tozeroy',
                        fillcolor: 'rgba(239,159,39,0.18)',
                        fillpattern: _hatch('rgba(226,75,74,0.7)', 'rgba(239,159,39,0.0)'),
                        line: { color: 'rgba(0,0,0,0)', width: 0 },
