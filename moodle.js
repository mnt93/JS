...
/*  @param title   {string}  Titre du graphique (texte plain)
...
function plot_distribution_plotly(div_id, law, params, x_obs, alpha, side, title, options) {
    ...
    /* ------------------------------------------------------------------ */
    /* Valeur symetrique (test bilateral)                                   */
    /* ------------------------------------------------------------------ */
    var x_sym = NaN;
    if (side === 'bilateral') {
        if (is_discrete) {
    ...
    /* ------------------------------------------------------------------ */
    /* Annotations Plotly (valeur observee + valeur(s) critique(s))        */
    /* Fleche strictement verticale : ax=0, axref/ayref='pixel'            */
    /* Le label LaTeX evite d'imbriquer x_title (deja $...$) dans $...$   */
    /* On construit le label comme texte plain + valeur numerique          */
    /* ------------------------------------------------------------------ */
    var annotations = [];

    /* Retire les $ entourant x_title pour l'imbriquer dans une nouvelle expr LaTeX */
    ...
    var _make_ann = function(xv, y_anchor, label, color, ay_px) {
        return {
            x: xv,
            y: y_anchor,
            xref: 'x',
            ...
            arrowhead: 2,
            arrowcolor: color,
            ax: 0,           /* horizontal : zero => fleche strictement verticale */
            ay: ay_px,       /* offset vertical en pixels (negatif = vers le haut) */
            axref: 'pixel',
            ayref: 'pixel',
    ...
    if (!isNaN(x_obs)) {
        ...
        var lbl_obs = '$' + _bare(x_title) + '_{\\text{obs}} = ' + (Math.round(x_obs * 1000) / 1000) + '$';
        annotations.push(_make_ann(x_obs, y_obs, lbl_obs, C_OBS, -45));
    }
    if (!isNaN(x_crit_lo)) {
        var y_clo = pdf_fn(x_crit_lo);
        if (!isFinite(y_clo) || isNaN(y_clo)) y_clo = 0;
        var lbl_clo = '$' + _bare(x_title) + '_{\\text{crit}} = ' + (Math.round(x_crit_lo * 1000) / 1000) + '$';
        annotations.push(_make_ann(x_crit_lo, y_clo, lbl_clo, C_CRIT, -45));
    }
    if (!isNaN(x_crit_hi)) {
        var y_chi = pdf_fn(x_crit_hi);
        if (!isFinite(y_chi) || isNaN(y_chi)) y_chi = 0;
        var lbl_chi = '$' + _bare(x_title) + '_{\\text{crit}} = ' + (Math.round(x_crit_hi * 1000) / 1000) + '$';
        annotations.push(_make_ann(x_crit_hi, y_chi, lbl_chi, C_CRIT, -45));
    }
    ...
    if (is_discrete) {
        ...
        while (_lte(k_iter, Math.round(x_max))) {
            ks_arr.push(k_iter);
            pmf_arr.push(pdf_fn(k_iter));

            var is_rej = (!isNaN(x_crit_lo) && _lte(k_iter, x_crit_lo))
                      || (!isNaN(x_crit_hi) && _gte(k_iter, x_crit_hi));
            var is_obs = (!isNaN(x_obs) && k_iter === Math.round(x_obs));
            var is_sym = (!isNaN(x_sym) && k_iter === Math.round(x_sym));

            if (is_obs || is_sym) {
                col_arr.push(C_OBS);
            } else if (is_rej) {
                col_arr.push('rgba(226,75,74,0.70)');
            } else {
                col_arr.push('rgba(55,138,221,0.50)');
            }
            k_iter++;
        }
        ...
        var trace = {
            ...
            marker: { color: col_arr, line: { color: 'rgba(0,0,0,0)', width: 0 } },
            hovertemplate: x_title + ' = %{x}<br>' + y_title + ' = %{y:.5f}<extra></extra>'
        };

        Plotly.newPlot(div_id, [trace], layout, plotly_opts);
    ...
    /* ------------------------------------------------------------------ */
        ...
        /* Trace de base : courbe + remplissage non-rejet */
        var traces = [{
            x: xs, y: ys,
            type: 'scatter', mode: 'lines', fill: 'tozeroy',
            fillcolor: C_NOREJECT,
            line: { color: C_CURVE, width: 2 },
            hovertemplate: x_title + ' = %{x:.4f}<br>' + y_title + ' = %{y:.5f}<extra></extra>'
        }];
        ...
        if (!isNaN(x_obs)) {
            ...
            if (!isFinite(y_obs_top) || y_obs_top === 0) {
                /* fallback : 90% du max de la courbe */
                var y_max_c = 0;
                var mm = 0;
                while (_lt(mm, ys.length)) { if (_gt(ys[mm], y_max_c)) y_max_c = ys[mm]; mm++; }
                y_obs_top = y_max_c * 0.9;
            }
            traces.push({
        ...
        Plotly.newPlot(div_id, traces, layout, plotly_opts);
    }
}
/*==============================================================================================================================*/
