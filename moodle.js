...
function plot_distribution_plotly(div_id, law, params, x_obs, alpha, side, title, options) {
    ...
    /* ------------------------------------------------------------------ */
        ...
        var _add_extreme_zone = function(xa, xb) {
            ...
            if (!xZ.length) return;
            /* Polygone ferme : montee gauche + courbe + descente droite + retour bas
               Le retour bas (xb->xa sur y=0) est trace explicitement pour
               que le trait inferieur soit visible au-dessus de l'axe. */
            var contour_x = [xa].concat(xZ).concat([xb, xa]);
            var contour_y = [0].concat(yZ).concat([0,  0]);
            extreme_traces.push({
                x: contour_x,
                y: contour_y,
