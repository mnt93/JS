...
function plot_distribution_plotly(div_id, law, params, x_obs, alpha, side, title, options) {
    ...
    /* ------------------------------------------------------------------ */
    /* Legendes (traces fictifs pour la legende Plotly)                    */
    /* ------------------------------------------------------------------ */
    var legend_traces = [
        {
            x: [null], y: [null], type: 'scatter', mode: 'markers',
            marker: { color: C_REJECT, size: 14, symbol: 'square' },
    ...
    if (!isNaN(x_obs)) {
        legend_traces.push({
            x: [null], y: [null], type: 'scatter', mode: 'markers',
            marker: { color: C_OBS, size: 10, symbol: 'line-ns', line: { color: C_OBS, width: 2 } },
    ...
    if (!isNaN(x_sym)) {
        legend_traces.push({
            x: [null], y: [null], type: 'scatter', mode: 'markers',
            marker: { color: C_SYM, size: 10, symbol: 'line-ns', line: { color: C_SYM, width: 2 } },
            name: _plain_ann('sym', x_sym),
            showlegend: true, hoverinfo: 'skip'
        });
    }
    if (!isNaN(x_crit_lo) || !isNaN(x_crit_hi)) {
        ...
        legend_traces.push({
            x: [null], y: [null], type: 'scatter', mode: 'markers',
            marker: { color: C_CRIT, size: 10, symbol: 'line-ns', line: { color: C_CRIT, width: 2 } },
    ...
    /* Pas de couleur speciale pour x_obs — seul le coloriages rejet/non-  */
    /* rejet est utilise, plus x_sym en violet si test bilateral            */
    /* ------------------------------------------------------------------ */
    if (is_discrete) {
