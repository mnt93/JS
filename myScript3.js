...
function valeurs_a_droite(x,rang,Nb_reponses,n,sci){ // combien de valeurs possibles entre x0 et xmax pour n chiffres significatifs
	x=significant_digits(x,n,sci);
    coeff=1;
    vad = new Array;
    for (r=rang+1;r<=Nb_reponses;r++){
    	d=Math.pow(10,Math.ceil(Math.log10(x)))/Math.pow(10,(n-Math.floor(x/Math.pow(10,Math.ceil(Math.log10(x))))));
        o=Math.ceil(Math.log10(x));
    	vad[r-rang-1]=significant_digits(x*1+coeff*d,n,sci)
        coeff=coeff+1
...
function valeurs_a_gauche(x,rang,n,sci){ 
	x=significant_digits(x,n,sci);
    coeff=1;
    vag = new Array;
    d=Math.pow(10,Math.ceil(Math.log10(x)))/Math.pow(10,(n-Math.floor(x/Math.pow(10,Math.ceil(Math.log10(x))))));
    ...
    for (r=rang-2;r>=0;r--){
    	vag[r]=significant_digits(x*1-coeff*d,n,sci)
        coeff=coeff+1
...
function getArray(x,n,rang,Nb_reponses,sci){
	vag=valeurs_a_gauche(x,rang,n,sci);
    vad=valeurs_a_droite(x,rang,Nb_reponses,n,sci);
    reponse = new Array;
