...
// Fonction calculant l'opposé du logarithme de la vraissemblance pour la régression logistique
function minus_log_likelihood(beta, X, y) {
...
function newtonMethod(f, x0, tol = 1e-6, maxIter = 100) {
  ...
  while (iter < maxIter) {
    ...
    const gradFx = partial_derivatives(f, x); //gradient de f en x
    const hessFx = hessian_matrix(f, x); //matrice hessienne de f en x
...
/*==============================================================================================================================*/
 ...
 function partial_derivatives(f, x) {
   const h = 1e-6;
   if (!Array.isArray(x)) {
   ...
   for (let i = 0; i < n; i++) {
     ...
     hVec[i] = h;
     if (n != 1) {
       grad[i] = (f(x.map((xi, j) => xi + hVec[j])) - f(x.map((xi, j) => xi - hVec[j]))) / (2 * h);
     } else {
       grad = (f(x[0] + h) - f(x[0])) / (2 * h);
     }
   }
   return grad;
 }
...
function hessian_matrix(f, x) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = i; j < n; j++) {
      ...
      if (n != 1) {
        hVecJ[j] = h;
        hess[i][j] = (f(x.map((xi, k) => xi + hVecI[k] + hVecJ[k])) - f(x.map((xi, k) => xi + hVecI[k] - hVecJ[k])) - f(x.map((xi, k) => xi - hVecI[k] + hVecJ[k])) + f(x.map((xi, k) => xi - hVecI[k] - hVecJ[k]))) / (4 * h ** 2);
        hess[j][i] = hess[i][j];
      } else {
        hess = (f(x[0] + h) - 2 * f(x[0]) + f(x[0] - h)) / (h * h);
...
function conf_int_logistic_reg(beta0) {
  let conf_int = new Array(beta.length).fill(0);
  let SD = new Array(beta.length).fill(0);
  for (let j = 0; j < beta0.length; j++) {
    SD[j] = math.sqrt(math.diag(math.inv(hessian_matrix(minus_log_likelihood_x, beta0))))[j]
    let teta0 = beta0[j];
    ...
    let profile_likelihood = function(teta) {
      let h = function(gamma) {
        let beta = gamma;
        beta.splice(j, 0, teta) // insérer teta dans beta à la position j
        let ff = Math.pow(minus_log_likelihood_x(beta), 20);
        return ff
      };
      let gamma = newtonMethod(h, gamma0); // cherche la valeur des coeff de beta correspondant au maximum de vraissemblance lorsque la valeur du jeme coeff de beta vaut teta.
      let beta = gamma;
      beta.splice(j, 0, teta) // insérer teta dans beta à la position j
      return math.pow((minus_log_likelihood_x(beta) - minus_log_likelihood_x(beta0) - jStat.chisquare.inv(0.95, 1) / 2), 2);
    };
	conf_int[j]=new Array(2).fill(0);

    conf_int[j][0] = newtonMethod(profile_likelihood, teta0 - 2 * SD[j]);
    conf_int[j][1] = newtonMethod(profile_likelihood, teta0 + 2 * SD[j]);
  }
  return conf_int
}
...
function Create_Log_Reg_Data(N, beta, mean_X, sigma_X, Col_Names, myrng) {
  ...
  var x = Array.from(Array(N), () => new Array(K - 1).fill(0));
  for (let i = 0; i < N; i++) {
    for (let j = 0; j < K - 1; j++) {
  ...
  var P = X.map((xi) => 1 / (1 + math.exp(math.multiply(-1, math.dot(xi, beta)))))

  var y = new Array(N).fill(1);
  for (let i = 0; i < N; i++) {
    if (myrng() > P[i]) {
      y[i] = 0;
    }
  ...
  var Y = math.subset(data_num, math.index(math.range(0, N), Col_Names.length - 1))
  var X = math.subset(data_num, math.index(math.range(0, N), math.range(0, Col_Names.length - 1)))
