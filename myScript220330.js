...
return IntervalArray
 ...
 function getRandomIntInclusive(min, max, myrng) {
    ...
    function Create_MLR_Data(rows, Intercept, coeff_X, mean_X, sigma_X, sigma_Y, Col_Names, Seed_Table) {
    	...
    	data_num=math.subset(data, math.index(math.range(1, rows+1), math.range(0, Col_Names.length)))
		Y=math.subset(data_num, math.index(math.range(0, rows), Col_Names.length-1)) 
    	X=math.subset(data_num, math.index(math.range(0, rows), math.range(0,Col_Names.length-1)))
    ...
    function MLR(Y, X) {
        ...
        X = math.subset(data_num, math.index(math.range(0, n), p), math.ones(n, 1)) // ajoute colonne de 1 pour estimer le terme constant
        beta = math.multiply(math.inv(math.multiply(math.transpose(X), X)), math.transpose(X), Y);
        Ypred = math.multiply(X, beta);
        Epred = math.subtract(Y, Ypred);
        sigma_E_estimat = math.sqrt(math.multiply(math.transpose(Epred), Epred) / (n - p - 1))
        sigma_Beta_estimat = math.diag(math.sqrt(math.multiply(math.diag(math.diag(math.inv(math.multiply(math.transpose(X), X)))), sigma_E_estimat * sigma_E_estimat)))
        var mlr_result = {
            'beta': beta,
            'Ypred': Ypred,
            'Epred': Epred,
    ...
    function CreateTable(Data,Table_position='',Table_id='table1') {
