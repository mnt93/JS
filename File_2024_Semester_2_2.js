...
function getArray(x, min_x, max_x, NCS, myrng, NbRep, rang, Num_question) {
  ...
  if (Math.abs(x) > 1e-3 && Math.abs(x) < 1e3) {
    proposedValuesNum = proposedValuesNum.map(value => math.format(value, NCS));
  } else {
  ...
  let reponsesIncorrectesStr = sample(proposedValuesNum, NbRep, myrng);
