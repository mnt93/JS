...
function Chi2_Test(X) {
    ...
    function display_polynom_LaTex(expr, tag_id) {
        document.getElementById(tag_id).appendChild(MathJax.tex2svg(math.parse(expr).toTex(), {display: false}));
    }
    ...
    function display_polynom(expr, tag_id, var_symb = 'x') {
        ...
        function plot_line(array,tag_id) {
        ...
        maxZoomIn: 1000.0
            ...
            var chart = new google.visualization.LineChart(document.getElementById(tag_id));
...
function getArray(x,n,rang,Nb_reponses,sci){
    ...
    if (d.getTime() < Date.parse("2022-09-01")) {
            ...
            for (j = 0; j < Nb_reponses; j++) {
                ...
                if (isNaN(Number(nerdamer(exprf_im).evaluate())) || Number(exprf_im) == 0 || Number(exprf_re) == 0) {
                    ...
                    if (Number(exprf_re) == 0) {
                        reponse[j] = '\\(\\ \\ \\ \\ ' + exprf_tex.replaceAll('\\cdot','') + ' \\)'
                    } else {
                        reponse[j] = '\\(\\ \\ \\ \\ ' + exprf_re_tex.replaceAll('\\cdot','') + ' \\)'
                    }
                } else {
                    if (Number(nerdamer(exprf_im).evaluate()) < 0) {
                        ...
                        exprf_re_tex = nerdamer(exprf_re).toTeX()
                    	reponse[j] = '\\(\\ \\ \\ \\ ' + exprf_re_tex.replaceAll('\\cdot','') + ' \\)' + '\\(  - \\)' + (nerdamer(exprf_im).toString().includes("+") ? ' \\((' + exprf_im_tex.replaceAll('\\cdot','') + ') i \\) ' : ' \\(' + exprf_im_tex.replaceAll('\\cdot','') + ' i \\) ')
                    } else {
                    	exprf_re_tex = nerdamer(exprf_re).toTeX()
                    	exprf_im_tex = nerdamer(exprf_im).toTeX()
                    	reponse[j] = '\\(\\ \\ \\ \\ ' + exprf_re_tex.replaceAll('\\cdot','') + ' \\)' + '\\(  + \\)' + (nerdamer(exprf_im).toString().includes("+") ? ' \\((' + exprf_im_tex.replaceAll('\\cdot','') + ') i \\) ' : ' \\(' + exprf_im_tex.replaceAll('\\cdot','') + ' i \\) ')
